...
def run(args):
    ...
    for step, batch in enumerate(loader):
        ...
        for b, oid in enumerate(batch['object_ids']):
            ...
            pair_pred.extend(pd)
            pair_gt.extend(gd)
            records.append({'object_id': oid, 'prediction_pair_mae': float(np.mean(pd)) if pd else None,
    ...
    np.savez_compressed(destination.with_suffix('.npz'), **arrays, targets=gt, affordance_ids=aids, category_ids=cats)
